...
function Get-OABInfo () {
    ...
    $UserHives | ForEach-Object {
        if (Test-Path "HKU:\$_\Software\Microsoft\Exchange\Exchange Provider\OABs\$OABGuid") {
            $OAB = New-Object -TypeName psobject
            $data = Get-ItemProperty "HKU:\$_\Software\Microsoft\Exchange\Exchange Provider\OABs\$OABGuid" | Select-Object -ExpandProperty 'OAB Last Modified Time'
            ...
            $OAB | Add-Member -MemberType NoteProperty -Name Name -Value (Get-ItemProperty "HKU:\$_\Software\Microsoft\Exchange\Exchange Provider\OABs\$OABGuid" | Select-Object -ExpandProperty 'OAB NameU')
